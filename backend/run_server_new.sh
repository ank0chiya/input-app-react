...
SERVER_CODE_DIR_NAME="output_flask_server_new"

# Docker イメージに関する設定
DOCKER_IMAGE_NAME="my-flask-mock-server_new" # ビルドするDockerイメージの名前
DOCKER_IMAGE_TAG="latest"                # イメージのタグ

# Docker コンテナに関する設定
CONTAINER_NAME="flask-mock-container_new"    # 起動するコンテナの名前
HOST_PORT="8081"                         # ホストマシンで公開するポート番号
